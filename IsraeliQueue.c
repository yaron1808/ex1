...
IsraeliQueue IsraeliQueueCreate(FriendshipFunction *friendshipFunc, ComparisonFunction compareFunc, int a, int b)
{
    int x = friendshipFunc[0](NULL, NULL);
    int y = compareFunc(NULL, NULL);
    int z = a + b;

    IsraeliQueue iq = NULL;
    return iq;
}
 ...
 * the parameter.*/
void IsraeliQueueDestroy(IsraeliQueue iq)
{
    iq = NULL;
    return;
}
...
IsraeliQueueError IsraeliQueueEnqueue(IsraeliQueue iq, void *ptr)
{
    iq = NULL;
    ptr = NULL;
